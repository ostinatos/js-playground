...
// still da.length === 5
console.debug(
    'delete element[1] using delete, length will not be changed: ',
    da
);
delete da[4];
console.debug(
    'delete element[4] using delete, length will not be changed either: ',
    da
);
...
console.debug(
    'splice(), do not delete, append elements at the end of the array ',
    spliceArray
);
...
console.debug(
    'map array: ',
    mapArray.map((element) => element * 2)
);
...
console.debug(
    'filter array: ',
    filterArray.filter((element) => element % 2 === 0)
);
...
console.debug(
    ...
    [1, 2, 3, 4, 5].reduce((prev, curr) => prev + curr)
);

// example: count the appearance of elements
console.debug(
    'count the appearance of elements ',
    ['a', 'b', 'c', 'b', 'a', 'd', 'e', 'c', 'b'].reduce(
        (summary, curr) => {
            if (curr in summary) {
                summary[curr]++;
            } else {
